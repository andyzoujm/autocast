...
def main():
    ...
    def get_confidence_intervals(logits):
        ...
        lower_deltas, higher_deltas = (
            deltas[:, :num_intervals],
            deltas[:, num_intervals:],
        )
        ...
        lower_deltas, higher_deltas = utils.cumsum(lower_deltas), utils.cumsum(
            higher_deltas
        )
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            (
                ...
                point_estimates,
                interval_lengths,
            ) = get_confidence_intervals(logits)
            containment, ci_error = utils.evaluate(
                lowers, uppers, labels, confidence_levels
            )
            low_containment_mask = ci_error < 0
            train_containment_tensors.append(containment)

            point_estimate_loss = pe_criterion(point_estimates, labels[:, 0:1]) / (
                1 + torch.abs(labels)
            )
            train_pe_dist.append(
                torch.abs(torch.exp(point_estimates) / torch.exp(labels[:, 0:1]) - 1)
                .median()
                .item()
            )
            ...
            miscalibration_loss = (
                (
                    pe_criterion(lowers, labels) * (lowers > labels)
                    + pe_criterion(uppers, labels) * (uppers < labels)
                )
                / (1 + torch.abs(labels))
            ).mean(dim=0)
            interval_length_loss = interval_criterion(
                interval_lengths,
                torch.zeros_like(interval_lengths, device=interval_lengths.device),
            ) / (1 + torch.abs(labels))
            train_interval_length.append(
                ((torch.exp(uppers) - torch.exp(lowers)) / torch.exp(labels[:, 0:1]))
                .median(dim=0)
                .values.detach()
                .cpu()
            )
            loss = (
                point_estimate_loss.mean()
                + (low_containment_mask * miscalibration_loss).mean()
                + lam
                * (~low_containment_mask * interval_length_loss.mean(dim=0)).mean()
            )
        ...
        train_adaptive_rms = utils.adaptive_binning_rms(
            train_containment_tensors, train_labels, confidence_levels
        )
        logger.info(f"epoch {epoch} | train rms: {utils.round_tensor(train_metric, 1)}")
        logger.info(
            f"epoch {epoch} | train rms: {utils.rms(train_metric, confidence_levels):.2f} | pe dist: {np.median(train_pe_dist):.2f} | interval len: {np.mean(np.median(torch.stack(train_interval_length), axis=0)):.2f} | lengths: {np.around(np.median(torch.stack(train_interval_length), axis=0), 2)}"
        )
        logger.info(
            f"epoch {epoch} | train binning rms: {utils.round_tensor(train_adaptive_rms, 1)}"
        )
        logger.info(
            f"epoch {epoch} | train binning rms: {np.mean(train_adaptive_rms.numpy()):.2f}"
        )
            ...
            for step, batch in enumerate(eval_dataloader):
                ...
                pred_results.extend(
                    [
                        list(row)
                        for row in zip(
                            point_estimates.detach().cpu().tolist(),
                ...
                eval_pe_dist.append(
                    torch.abs(
                        torch.exp(point_estimates) / torch.exp(labels[:, 0:1]) - 1
                    )
                    .median()
                    .item()
                )
                # interval_length_loss = criterion(interval_lengths, torch.zeros_like(interval_lengths, device=interval_lengths.device))
                eval_interval_length.append(
                    (
                        (torch.exp(uppers) - torch.exp(lowers))
                        / torch.exp(labels[:, 0:1])
                    )
                    .median(dim=0)
                    .values.detach()
                    .cpu()
                ...
                extra_eval_interval_length.append(
                    (
                        (torch.exp(uppers) - torch.exp(lowers))
                        / torch.exp(labels[:, 0:1])
                    )
        ...
        eval_adaptive_rms = utils.adaptive_binning_rms(
            eval_containment_tensors, eval_labels, confidence_levels
        )
        logger.info(f"epoch {epoch} | eval rms: {utils.round_tensor(eval_metric, 1)}")
        logger.info(
            f"epoch {epoch} | eval rms: {utils.rms(eval_metric, confidence_levels):.2f} | pe dist: {np.median(eval_pe_dist):.2f} | interval len: {np.mean(np.median(torch.stack(eval_interval_length), axis=0)):.2f} | lengths: {np.around(np.median(torch.stack(eval_interval_length), axis=0), 2)}"
        )
        logger.info(
            f"epoch {epoch} | eval binning rms: {utils.round_tensor(eval_adaptive_rms, 1)}"
        )
        logger.info(
            f"epoch {epoch} | eval binning rms: {np.mean(eval_adaptive_rms.numpy()):.2f}\n"
        )
